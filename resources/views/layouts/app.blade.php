        ...
        <aside class="w-64 bg-white border-r border-pink-100 flex-shrink-0 hidden md:flex flex-col shadow-sm">
            ...
            <nav class="flex-1 overflow-y-auto py-4 px-4 space-y-1">
                ...
                @if (Auth::user()->role == 'admin')
                    ...
                    <a href="{{ route('admin.laporan.index') }}"
                        class="flex items-center p-3 text-sm font-semibold rounded-2xl transition-all {{ request()->routeIs('admin.laporan.index') ? 'bg-pink-600 text-white shadow-lg' : 'text-gray-600 hover:bg-pink-50' }}">
                ...
                @else
                    ...
                    <a href="{{ route('orders.index') }}"
                        class="flex items-center p-3 text-sm font-semibold rounded-2xl transition-all {{ request()->routeIs('orders.*') ? 'bg-pink-600 text-white shadow-lg' : 'text-gray-600 hover:bg-pink-50' }}">
            ...
            <div class="p-4 border-t border-pink-50 bg-pink-50/30">
                <div class="flex items-center space-x-3">
                    <div
                        class="w-8 h-8 rounded-full bg-pink-600 flex items-center justify-center text-white text-xs font-bold uppercase">
                        {{ substr(Auth::user()->name, 0, 1) }}
                    </div>
                    <div class="overflow-hidden">
                        <p class="text-xs font-bold text-gray-800 truncate">{{ Auth::user()->name }}</p>
                        <p class="text-[10px] text-pink-600 font-medium uppercase tracking-tighter">
                            {{ Auth::user()->role }}</p>
                    </div>
                </div>
            </div>
