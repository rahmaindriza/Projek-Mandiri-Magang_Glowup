...
<body class="font-sans antialiased bg-slate-50">
    ...
    <a href="{{ route('admin.dashboard') }}" class="flex items-center p-3 text-sm font-semibold rounded-2xl transition-all {{ request()->routeIs('admin.dashboard') ? 'bg-pink-600 text-white shadow-lg' : 'text-gray-600 hover:bg-pink-50' }}">
    ...
    @if(Auth::user()->role == 'admin')
        ...
        <a href="{{ route('admin.products.index') }}" class="flex items-center p-3 text-sm font-semibold rounded-2xl transition-all {{ request()->routeIs('admin.products.index') ? 'bg-pink-600 text-white shadow-lg' : 'text-gray-600 hover:bg-pink-50' }}">
        ...
        <a href="{{ route('admin.orders.index') }}" class="flex items-center p-3 text-sm font-semibold text-gray-600 hover:bg-pink-50 rounded-2xl transition-all">
    ...
    <span class="mr-3 text-lg">🛒</span> Keranjang Saya
        ...
        <a href="{{ route('orders.index') }}" class="flex items-center p-3 text-sm font-semibold text-gray-600 hover:bg-pink-50 rounded-2xl transition-all">
            ...
            <div class="p-4 border-t border-pink-50 bg-pink-50/30">
                <div class="flex items-center space-x-3">
                    ...
                    <div class="overflow-hidden">
                        ...
                        <p class="text-[10px] text-pink-600 font-medium">Administrator</p>
                    </div>
                </div>
            </div>
        </aside>
    ...
    <script>
        // Notifikasi Berhasil (Tambah/Hapus/Update)
        @if(session('success'))
            Swal.fire({
                icon: 'success',
                title: 'Berhasil! ✨',
                ...
                color: '#4b5563',
                customClass: {
                    popup: 'rounded-[2rem] shadow-xl border border-pink-50',
                    title: 'font-playfair text-pink-600',
                }
            });
        @endif

        // Notifikasi Error (Jika ada kendala)
        @if(session('error'))
            Swal.fire({
                icon: 'error',
                title: 'Ups!',
