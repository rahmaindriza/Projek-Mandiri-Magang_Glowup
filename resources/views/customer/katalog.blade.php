                ...
                <form action="{{ route('customer.katalog') }}" method="GET" class="relative group max-w-md mx-auto">
                    ...
                    @if (request('search'))
                        <a href="{{ route('customer.katalog') }}"
                            class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-pink-600 font-bold text-xs uppercase tracking-widest">
                            Reset
                        </a>
                    @endif
                </form>
                ...
                @forelse($products as $product)
                    <div
                        ...
                        <div class="px-2 text-center">
                            ...
                            <div class="px-2 text-center space-y-2 mt-4">
                                {{-- TOMBOL TAMBAH KERANJANG (Tetap di halaman) --}}
                                <form action="{{ route('cart.store', $product->id) }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="redirect_to" value="index">
                                    <button type="submit"
                                        class="w-full bg-gray-900 text-white py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-gray-800 transition">
                                        + Tambah Ke Keranjang
                                    </button>
                                </form>
                                ...
                                <form action="{{ route('cart.store', $product->id) }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="redirect_to" value="checkout"> {{-- HARUS 'checkout' --}}
                                    <button type="submit"
                                        class="w-full bg-pink-600 text-white py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-pink-700 shadow-lg shadow-pink-100 transition">
                                        🛍️ Beli Sekarang
                                    </button>
                                </form>
                            </div>
