                    ...
                    <div class="mb-8 p-6 bg-white border-l-8 border-red-500 rounded-3xl shadow-sm animate-pulse">
                        <div class="flex items-center justify-between flex-wrap gap-4">
                            <div class="flex items-center">
                                ...
                                <div>
                                    <h4 class="text-lg font-black text-red-700 uppercase tracking-tight">Peringatan Stok Kritis!</h4>
                                    <p class="text-sm text-gray-500 font-medium">Ada {{ $lowStockProducts->count() }} produk skincare dengan stok di bawah 5 pcs.</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                            @foreach($lowStockProducts as $low)
                                <div class="flex items-center p-3 bg-pink-50/50 rounded-2xl border border-red-100">
                                    <img src="{{ asset('storage/'.$low->image) }}" class="w-12 h-12 object-cover rounded-xl mr-3 border border-white">
                                    <div class="flex-1">
                                        <p class="text-xs font-bold text-gray-800 line-clamp-1">{{ $low->name }}</p>
                                        <p class="text-[10px] text-red-600 font-black italic">Sisa: {{ $low->stock }} pcs</p>
                                    </div>
                                    <a href="{{ route('admin.products.addStock', $low->id) }}" class="ml-2 p-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition shadow-sm">
                                ...
                                @foreach($orders as $order)
                                    <tr class="hover:bg-pink-50/30 transition-colors">
                                        ...
                                        <td class="px-4 py-4 text-center">
                                            <span class="px-3 py-1 rounded-full text-[9px] font-black uppercase {{ $order->status == 'success' ? 'bg-green-100 text-green-600' : 'bg-yellow-100 text-yellow-600' }}">
                                                {{ $order->status }}
                                            </span>
                                        </td>
                                    </tr>
